"""Evaluation metrics.
  ...
  - citation_integrity: every [source_id] resolves to a real source.
  - keyword recall (factual only): expected keywords appear in the report;
    only meaningful with a real search backend.
  - honesty (unanswerable only): uncertainties surfaced or low confidence.
  - grounding_precision: LLM judge re-checks a sample of supported claims;
    costs LLM calls, can be disabled in the runner.
"""
...
def report_text(report: FinalReport) -> str:
    """Recommendation plus section bodies."""
...
def citation_integrity(report: FinalReport) -> tuple[bool, bool, list[str]]:
    ...
    citations passes vacuously; `has_citations` makes that visible.
    """
...
def honesty_ok(report: FinalReport) -> bool:
    """Honest on an unanswerable question = flags uncertainty or stays low-confidence."""
...
def grounding_precision(
    ...
    Fail-closed: a claim whose judgment call errors counts as ungrounded.
    """
    if not pairs:
        return None, 0
