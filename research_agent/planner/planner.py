...
_SYSTEM = (
    ...
    "independent, concrete sub-tasks that can each be answered by a single web search. "
    "Prefer 2-5 sub-tasks. Only add a dependency when a sub-task genuinely needs an "
    "earlier one's result. Return STRICT JSON only."
)
...
{{
  "sub_tasks": [
    {{"id": "t1", "description": "<one concrete, searchable sub-question>", "depends_on": []}},
    {{"id": "t2", "description": "...", "depends_on": ["t1"]}}
  ]
}}

Rules:
- 2 to 5 sub-tasks. Each description is self-contained and searchable on its own.
...
def _build_plan_from_json(question: str, data: Any) -> Plan:
    ...
    for item in raw_tasks[:_MAX_SUB_TASKS]:
        ...
        if not desc:
            continue
        sub_tasks.append(SubTask(description=desc, tool_hint=SourceType.WEB))
